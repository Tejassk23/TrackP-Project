...
def dashboard():
    ...
    total_cases = 0
    total_missing_persons = 0
    total_found_persons = 0

    if conn:
        cursor = conn.cursor()

        # Fetch total cases count for the specific station
        cursor.execute("SELECT COUNT(*) FROM missing_person WHERE station_id = %s", (station_id,))
        total_cases = cursor.fetchone()[0]

        # Fetch total missing persons count (status = 'missing') for the specific station
        cursor.execute("SELECT COUNT(*) FROM missing_person WHERE station_id = %s AND status = 'missing'", (station_id,))
        total_missing_persons = cursor.fetchone()[0]

        # Fetch total found persons count (status = 'found') for the specific station
        cursor.execute("SELECT COUNT(*) FROM missing_person WHERE station_id = %s AND status = 'found'", (station_id,))
        total_found_persons = cursor.fetchone()[0]
    ...
    return render_template(
        ...
        total_missing_persons=total_missing_persons,
        total_found_persons=total_found_persons
    )
...
def notify_station():
    ...
    identify_time = datetime.now().strftime('%H:%M:%S')  # Get the current time in HH:MM:SS format

    # Validate the required fields
    if not camera_id or not missing_person_id:
        return jsonify({"error": "camera_id and missing_person_id are required"}), 400
    ...
    try:
        ...
        # Insert the notification into the `history` table with current date and time
        cursor.execute("""
            INSERT INTO history (missing_person_id, camera_id, identify_date, time)
            VALUES (%s, %s, %s, %s)
        """, (missing_person["missing_person_id"], camera_id, identify_date, identify_time))
        conn.commit()
...
def time_12hr_format(value):
    """Convert a 24-hour time string to 12-hour format with AM/PM."""
    try:
        return datetime.strptime(value, "%H:%M:%S").strftime("%I:%M:%S %p")
    except ValueError:
...
def notification():
    ...
    try:
        ...
        # Query to fetch notifications from the `history` table
        query = """
        SELECT 
            h.id AS history_id,
            h.missing_person_id,
        ...
        LEFT JOIN 
            missing_person mp
        ON 
            h.missing_person_id = mp.id
        WHERE 
            mp.station_id = %s -- Filter by the station_id from the session
        ORDER BY 
            h.identify_date DESC, h.time DESC -- Order by date and time, latest first
        """
        ...
        if conn:
            try:
                ...
                cursor.execute(insert_query, (
                    station_id, name, birthdate, address, missingdate, complaintdate, criminal, aadharCardNo,
                    photo_path))
                conn.commit()
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if len(uploaded_encodings) != 1:
                return jsonify({"error": "Uploaded photo must contain exactly one face"}), 400

            uploaded_encoding = uploaded_encodings[0]

            # Check if reference_encodings is empty
            if not reference_encodings:
                return jsonify({"error": "No reference data available to compare"}), 500

            face_distances = face_recognition.face_distance(reference_encodings, uploaded_encoding)

            # Ensure face_distances is not empty before using np.argmin
            if len(face_distances) == 0:
                return jsonify({"error": "No faces in the reference data to compare"}), 500
            ...
            error_details = {"message": "An error occurred", "error": str(e)}
